...
- **PDFs** → run `mac-ocr <path>` (macOS) or `pdftotext -layout <path> -` (Linux) via Bash and read its text output. Do NOT use the Read tool on PDFs - Read renders each page to an image (token-heavy) and fails on scanned/image-only PDFs.
